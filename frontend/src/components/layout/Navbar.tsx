    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <span className="text-2xl font-display font-bold tracking-tighter bg-gradient-to-r from-white via-white to-primary bg-clip-text text-transparent">
            SquadFun
          </span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          {isConnected && balance && (
            <div className="hidden lg:flex flex-col items-end px-4 py-2 rounded-xl bg-white/5 border border-white/10">
              <span className="text-[10px] uppercase tracking-widest text-white/40 font-bold">Your Mana</span>
              <span className="font-mono text-sm font-bold text-primary-highlight">{formatMON(balance.value)}</span>
            </div>
          )}
          
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-monad/10 border border-monad/20">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[10px] font-mono font-bold text-monad uppercase">Monad Testnet</span>
          </div>
                ...
                >
                  {(() => {
                    if (!connected) {
                      return (
                        <button 
                          ...
                          className="bg-primary hover:bg-primary-bright text-white px-6 py-2.5 rounded-xl font-display font-bold transition-all shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:shadow-[0_0_30px_rgba(139,92,246,0.5)] active:scale-95"
                        >
                          Connect Wallet
                        </button>
                      );
                    ...
                    if (chain.unsupported) {
                      return (
                        <button onClick={openChainModal} className="bg-red-400 text-white px-6 py-2.5 rounded-xl font-display font-bold">
                          Wrong Network
                        </button>
                      );
                    }
                    ...
                    return (
                      ...
                      >
                        <div className="w-6 h-6 rounded-lg bg-gradient-to-tr from-primary to-primary-bright shadow-lg" />
